    ...
    def poll_once(self) -> None:
        ...
        for job_id, job in jobs.items():
            if not isinstance(job, dict) or job.get("deleted_by_client") or job.get("polling_disabled"):
                continue
            self._process_one(job_id, job)

    def kick(self, job_id: str) -> None:
        """Immediately process a single job without waiting for the next poll cycle.

        Called right after a job is created/finalized so cached torrents are staged
        and handed off to Sonarr within a second instead of waiting up to poll_interval.
        """
        job = self.store.get(job_id)
        if job and isinstance(job, dict) and not job.get("deleted_by_client") and not job.get("polling_disabled"):
            try:
                self._process_one(job_id, job)
            except Exception:
                logger.exception("KICK error torrent_id=%s", job_id)

    def _process_one(self, job_id: str, job: dict) -> None:
        ...
        try:
                        ...
                        if cmd_result == "successful":
                            ...
                            if actually_imported:
                                ...
                                if scan_fail_count >= 2 and arr_path and arr_client.is_configured():
                                    ...
                                    queued = arr_client.attempt_manual_import(Path(arr_path), download_client_id)
                                    if queued > 0:
                                        self.store.merge(job_id, {
                                            "status": "scan_pending",
                                            "arr_scan_command": {
                                        ...
                                        logger.info("IMPORT manual_import_submitted torrent_id=%s files=%d", job_id, queued)
                                        return
                                logger.warning(
            ...
            if mapped == "failed":
                ...
                self.store.merge(job_id, patch)
                logger.warning("POLL failed torrent_id=%s rd_status=%s reason=%s", job_id, info.get("status"), reason)
                return
            ...
            if mapped != "ready":
                if mapped == "stalled":
                    ...
                    if not stalled_since:
                        ...
                        self.store.merge(job_id, patch)
                        logger.warning("POLL stalled torrent_id=%s rd_status=%s; will fail in 2 minutes", job_id, info.get("status"))
                    else:
                        try:
                            from datetime import datetime, timezone as _tz
                        ...
                        if stalled_age >= 120:
                            ...
                            self.store.merge(job_id, patch)
                            logger.error("POLL stall_timeout torrent_id=%s stalled_for=%.0fs; failing job", job_id, stalled_age)
                        else:
                            patch["status"] = "stalled"
                            self.store.merge(job_id, patch)
                            logger.info("POLL stalled torrent_id=%s stalled_for=%.0fs/120s", job_id, stalled_age)
                else:
            ...
            if not source_file:
                ...
                logger.warning(
                    ...
                    job_id, info.get("filename"), self.settings.debrid_all_dir,
                )
                return
            ...
            host_ready, host_reason, host_details = check_staging_ready(
                staging_path, expected_media_size, self.settings.import_stability_min_bytes,
            )
            visible_ready, visible_reason, visible_details = check_staging_ready(
                visible_file, expected_media_size, self.settings.import_stability_min_bytes,
            )
            ...
            if patch["status"] == "staged":
                logger.info(
                    ...
                    job_id, patch.get("arr_ready_reason"), patch.get("arr_ready_details"),
                )
